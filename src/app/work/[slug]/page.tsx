...
export default async function CaseStudyPage({
  ...
  return (
    <article>
      <header className="pt-12 pb-16 sm:pt-16 sm:pb-24">
        <Container wide>
          <nav aria-label="Breadcrumb" className="anim-rise" style={{ ["--i" as string]: 0 }}>
            <ol className="type-label flex items-center gap-3 text-bone-500">
              <li><Link href="/work" className="link-underline">Projects</Link></li>
          ...
          <div className="mt-12 max-w-6xl">
            ...
            <h1
              className="anim-rise type-h1 mt-7 text-balance text-bone-50"
              style={{ ["--i" as string]: 2 }}
            >
              {project.title}
            </h1>
            <p
              className="anim-rise type-lead mt-8 max-w-3xl text-pretty text-bone-400"
              style={{ ["--i" as string]: 3 }}
            >
              {project.tagline}
            </p>
          ...
          <dl
            className="anim-rise mt-16 grid gap-x-10 gap-y-7 border-t border-bone-50 pt-7 md:grid-cols-3"
            style={{ ["--i" as string]: 4 }}
          >
            <div>
              <dt className="type-label text-bone-500">Context</dt>
              <dd className="type-meta mt-2 text-bone-200">{project.context}</dd>
            </div>
            <div>
              <dt className="type-label text-bone-500">Timeframe</dt>
              <dd className="type-meta mt-2 text-bone-200">{project.timeframe}</dd>
            </div>
            <div>
              <dt className="type-label text-bone-500">Contribution</dt>
              <dd className="type-meta mt-2 text-bone-200">{copy.contribution}</dd>
            </div>
          </dl>
        </Container>
      </header>
      ...
      <Container wide>
        ...
        <section className="py-16 sm:py-24">
          <Reveal>
            <CaseHeading index={nextIndex()}>Selected results</CaseHeading>
          </Reveal>

          {project.metrics.length > 0 && (
            <div className="mt-10 grid gap-8 border-y border-carbon-700 py-8 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {copy.choices.length > 0 && (
          <section className="py-16 sm:py-24">
            <Reveal>
              <CaseHeading index={nextIndex()}>Key design choices</CaseHeading>
            </Reveal>
            <ol className="mt-10 border-b border-carbon-700 lg:ml-[calc(12rem+1rem)]">
              {copy.choices.map((choice, index) => (
                <Reveal
                  ...
                  key={choice.title}
                  index={index}
                  className="grid gap-4 border-t border-carbon-700 py-7 lg:grid-cols-[minmax(0,18rem)_minmax(0,1fr)] lg:gap-10"
                >
        ...
        <section className="py-16 sm:py-24">
          <Reveal className="grid gap-10 border-t border-bone-50 pt-8 lg:grid-cols-2">
            <div>
              ...
              <div className="mt-5"><TagList items={project.stack} /></div>
            </div>
                  ...
                  {project.links.map((link) => (
                    <li key={link.href}>
                      <a
                        ...
                        className="type-meta link-underline text-bone-200"
                      >
                        {link.label} ↗
                      </a>
                    </li>
        ...
        <nav aria-label="More projects" className="grid gap-8 py-10 sm:grid-cols-2">
          {prev ? (
            <Link href={`/work/${prev.slug}`} className="group border-t border-carbon-700 py-5">
              <span className="type-label text-bone-500">← Previous</span>
              <span className="type-h3 mt-3 block text-bone-100 transition-opacity group-hover:opacity-60">
                {prev.title}
              </span>
            </Link>
          ) : <span />}
          {next && (
            <Link href={`/work/${next.slug}`} className="group border-t border-carbon-700 py-5 sm:text-right">
