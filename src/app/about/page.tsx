    ...
    <>
      <section className="pt-16 pb-16 sm:pt-24">
        <Container wide>
          ...
          <div className="mt-8 grid gap-14 lg:grid-cols-[minmax(0,1fr)_18rem] lg:gap-20">
            <div>
              <h1
                className="anim-rise type-h1 max-w-5xl text-balance text-bone-50"
                style={{ ["--i" as string]: 1 }}
              >
              ...
              <div className="mt-10 flex max-w-2xl flex-col gap-6">
                {bio.map((paragraph, index) => (
                  <p
                    key={paragraph}
                    className="anim-rise type-prose text-pretty text-bone-300"
            ...
            <aside
              className="anim-rise self-start border-t border-bone-50"
              style={{ ["--i" as string]: 5 }}
              aria-label="Details"
            >
              {[
                ...
                <div key={item.key} className="border-b border-carbon-700 py-4">
                  <span className="type-meta block text-bone-500">{item.key}</span>
                  {item.href ? (
                    <a
                      href={item.href}
                      className="link-underline type-meta mt-1 block w-fit break-all text-bone-100"
        ...
        <Container wide>
          <Reveal>
            <SectionHeader
              ...
              title="Tools used across projects and co-op terms."
              intro="Grouped by where I have the most hands-on depth, where I have shipped or built with a tool, and where I have prior familiarity."
            />
          </Reveal>

          <Reveal className="mt-10 flex flex-wrap gap-x-8 gap-y-3">
            {(["core", "working", "exposure"] as const).map((depth) => (
              <span key={depth} className={`type-label ${depthStyle[depth]}`}>
                {depthLabel[depth]}
              </span>
            ))}
          </Reveal>
              ...
              <Reveal key={group.title} index={index % 2}>
                ...
                <ul className="mt-6">
                  {group.items.map((item) => (
                    <li
                      key={item.name}
                      className="grid gap-x-6 gap-y-1 border-t border-carbon-700 py-4 sm:grid-cols-[minmax(0,11rem)_minmax(0,1fr)]"
                    >
                      ...
                      <div>
                        ...
                        {item.note && (
                          <span className="type-meta mt-1 block text-bone-400">{item.note}</span>
                        )}
                      </div>
                    </li>
                  ))}
        ...
        <Container wide>
          <Reveal>
            <SectionHeader
              index="02"
              label="Next"
              title="Areas I want to explore further."
              intro="A few directions connecting my current software, machine learning, and hardware experience."
            />
          </Reveal>
          ...
          <Reveal className="mt-12 flex flex-wrap gap-3">
            <Link href="/work" className="type-label bg-bone-100 px-6 py-4 text-carbon-850">
              View projects →
            </Link>
            <Link href="/contact" className="type-label border border-bone-100 px-6 py-4 text-bone-100">
              Contact
            </Link>
          </Reveal>
        </Container>
